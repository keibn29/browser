...
console.log('\n=== Launching Chrome Profile 2 (Concurrent Mode) ===\n')
...
const config = {
	...
	userDataDir:
		process.platform === 'darwin'
			? `${process.env.HOME}/Library/Application Support/Google/Chrome-Profile2`
			: process.platform === 'win32'
			? `${process.env.LOCALAPPDATA}\\Google\\Chrome-Profile2\\User Data`
			: `${process.env.HOME}/.config/google-chrome-profile2`,

	// Proxy configuration
	proxy: 'sunday.mikproxy.online:3134:21JUWA:sday', // Format: host:port:username:password
	...
	localProxyPort: 8889, // Local proxy relay port (unique)
	remoteDebuggingPort: 9223, // Remote debugging port (unique - different from Profile 1!)
...
console.log('[CONFIG] Profile 2 Configuration:')
console.log(`   Profile Name: ${config.profileName}`)
console.log(`   Proxy: ${config.proxy.split(':').slice(0, 2).join(':')}`)
...
runRealProfile(config).catch(error => {
	console.error('\n[ERROR] Failed to launch Profile 2:')
