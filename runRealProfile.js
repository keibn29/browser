...
export async function runRealProfile(config = {}) {
	...
	if (finalConfig.proxy) {
		try {
			...
			// Step 2.1: Start proxy relay server for authenticated proxy
			if (proxyConfig.hasAuth) {
	...
	if (isRunning && finalConfig.killExisting) {
		...
		console.log(
			'[WARN] Chrome is running. Proxy changes may not take effect.'
		)
	...
	// Step 4: Update Preferences if proxy is configured
	if (proxyConfig) {
	...
	try {
		...
		if (proxyConfig) {
			...
			if (proxyConfig.hasAuth) {
				...
				console.log(
					'   - Chrome connects to localhost without authentication'
				)
